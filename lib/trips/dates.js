...
function fromUTCDays(utcMs) {
  return new Date(utcMs).toISOString().slice(0, 10);
}

export function totalTripDays(startDate, endDate) {
  return Math.round((toUTCDays(endDate) - toUTCDays(startDate)) / MS_PER_DAY) + 1;
}

export function daysUntil(fromDateStr, toDateStr) {
  return Math.round((toUTCDays(toDateStr) - toUTCDays(fromDateStr)) / MS_PER_DAY);
}

export function computeStopDateRanges(startDate, dayCounts) {
  let cursor = toUTCDays(startDate);

  return dayCounts.map((count) => {
    const stopStart = fromUTCDays(cursor);
    const stopEnd = fromUTCDays(cursor + (count - 1) * MS_PER_DAY);
    cursor += count * MS_PER_DAY;
    return { startDate: stopStart, endDate: stopEnd };
  });
}
